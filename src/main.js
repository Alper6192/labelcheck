...
function renderLog(){el("logCount").textContent=`${records.length} Datensätze`;const body=el("logBody");body.replaceChildren();records.slice(0,30).forEach(r=>{const tr=document.createElement("tr");tr.innerHTML=`<td>${new Date(r.timestamp).toLocaleString()}</td><td>${r.product.batch||"–"}</td><td>${r.product.drum_number||"–"}</td><td>${r.vda.batch||"–"}</td><td>${r.result}</td>`;body.append(tr);});}
