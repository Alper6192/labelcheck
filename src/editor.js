...
document.querySelector("#editorApp").innerHTML = `
  ...
  <main class="profile-editor-layout">
    ...
    <section class="panel editor-canvas-panel">
      ...
      <div id="editorCanvasWrap" class="editor-canvas-wrap"><canvas id="editorCanvas"></canvas></div>
      <div class="hint">Florence einmal auf dem Masterbild ausführen. Danach die Kundenname-Textbox und die reinen Wertboxen anklicken. Alternativ „Bereich zeichnen“ verwenden.</div>
    </section>
...
const el = Object.fromEntries(["importConfig","exportConfig","newProfile","analyzeMaster","modeSelect","modeDraw","editorStatus","profileCount","profileSearch","profileList","canvasTitle","selectionInfo","masterInput","assignAnchor","editorCanvasWrap","editorCanvas","dirtyBadge","profileName","profileRole","anchorAliases","manualOnly","activeProfile","configuredProfile","assignmentSelect","selectedRegionText","fieldPattern","fieldRequired","fieldCompare","deleteAssignment","profileValidation"].map((id) => [id, document.getElementById(id)]));
...
async function selectProfile(id) {
  ...
  await renderCanvas(); renderProfileList(); renderAssignmentOptions(); renderSelectedRegion(); renderValidation();
}
...
async function analyzeMaster() {
  const profile = current(); if (!profile?.master?.image || state.busy) return;
  state.busy = true; setStatus("Florence wird geladen und liest das vollständige Masterbild …", "warn");
  try {
    const client = getClient(); await client.load();
    const response = await client.analyze(profile.master.image, profile.role);
    state.entries = parseFlorenceEntries(response.result, response.imageSize);
    profile.master.geometry = state.entries.map((entry) => ({ x: entry.centerX / response.imageSize[0], y: entry.centerY / response.imageSize[1], width: entry.width / response.imageSize[0], height: entry.height / response.imageSize[1] }));
    ...
    renderCanvas(); renderSelectedRegion(); renderValidation();
  } catch (error) { setStatus(`Florence-Fehler: ${error.message || error}`, "bad"); }
...
async function replaceMaster(event) {
  ...
  const image = await prepareImage(file); const profile = current();
  profile.master = { width: image.width, height: image.height, image: image.dataUrl, geometry: [] };
  state.entries = []; state.selectedEntry = null; state.drawnRect = null; markDirty(); renderAssignmentOptions(); renderCanvas(); renderValidation();
}
...
function setMode(mode) { state.mode = mode; state.drawnRect = null; el.modeSelect.classList.toggle("primary", mode === "select"); el.modeDraw.classList.toggle("primary", mode === "draw"); setStatus(mode === "select" ? "Florence-Textbox anklicken." : "Mit dem Finger oder der Maus ein Rechteck ziehen.", ""); renderCanvas(); }

function canvasPointerDown(event) {
  const point = canvasToImage(event); if (!point) return;
  if (state.mode === "select") {
    ...
    renderCanvas(); renderSelectedRegion(); return;
  }
  state.dragStart = point; state.dragCurrent = point; state.selectedEntry = null; el.editorCanvas.setPointerCapture(event.pointerId); renderCanvas();
}
function canvasPointerMove(event) { if (state.mode !== "draw" || !state.dragStart) return; state.dragCurrent = canvasToImage(event); renderCanvas(); }
function canvasPointerUp(event) {
  if (state.mode !== "draw" || !state.dragStart) return;
  const end = canvasToImage(event); const profile = current();
  ...
  state.drawnRect = { x:left/profile.master.width, y:top/profile.master.height, width:(right-left)/profile.master.width, height:(bottom-top)/profile.master.height };
  state.dragStart = null; state.dragCurrent = null; renderCanvas(); renderSelectedRegion();
}

async function renderCanvas() {
  const canvas = el.editorCanvas, wrap = el.editorCanvasWrap, profile = current();
  canvas.width = Math.max(500, wrap.clientWidth - 12); canvas.height = Math.max(430, wrap.clientHeight - 12);
  const context = canvas.getContext("2d"); context.fillStyle = "#06101d"; context.fillRect(0,0,canvas.width,canvas.height);
  if (!profile?.master?.image) { context.fillStyle="#9baec5"; context.textAlign="center"; context.fillText("Kein Masterbild",canvas.width/2,canvas.height/2); return; }
  const image = await loadImage(profile.master.image); const pad=16, scale=Math.min((canvas.width-pad*2)/image.width,(canvas.height-pad*2)/image.height); const offsetX=(canvas.width-image.width*scale)/2, offsetY=(canvas.height-image.height*scale)/2;
  context.drawImage(image,offsetX,offsetY,image.width*scale,image.height*scale); state.view={scale,offsetX,offsetY,width:image.width,height:image.height};
  for (const entry of state.entries) drawQuad(context,entry.box,"#ffa31a","rgba(255,163,26,.07)",scale,offsetX,offsetY);
  const anchor = profile.anchor?.masterQuad?.map((value,index)=>value*(index%2?profile.master.height:profile.master.width)); if(anchor) drawQuad(context,anchor,"#40d486","rgba(64,212,134,.12)",scale,offsetX,offsetY,"ANKER");
  for (const field of profile.fields||[]) { const quad=rectToQuad(field.rect).map((value,index)=>value*(index%2?profile.master.height:profile.master.width)); drawQuad(context,quad,"#55c2f2","rgba(85,194,242,.10)",scale,offsetX,offsetY,field.label||field.key); }
  if(state.selectedEntry?.box) drawQuad(context,state.selectedEntry.box,"#ffffff","rgba(255,255,255,.10)",scale,offsetX,offsetY,"AUSWAHL");
  if(state.drawnRect){const quad=rectToQuad(state.drawnRect).map((value,index)=>value*(index%2?profile.master.height:profile.master.width));drawQuad(context,quad,"#ffffff","rgba(255,255,255,.10)",scale,offsetX,offsetY,"NEU");}
  if(state.dragStart&&state.dragCurrent){const q=[state.dragStart.x,state.dragStart.y,state.dragCurrent.x,state.dragStart.y,state.dragCurrent.x,state.dragCurrent.y,state.dragStart.x,state.dragCurrent.y];drawQuad(context,q,"#ffffff","rgba(255,255,255,.08)",scale,offsetX,offsetY);}
}
...
function drawQuad(ctx,quad,stroke,fill,scale,ox,oy,label=""){ctx.beginPath();for(let i=0;i<8;i+=2){const x=ox+quad[i]*scale,y=oy+quad[i+1]*scale;i===0?ctx.moveTo(x,y):ctx.lineTo(x,y);}ctx.closePath();ctx.strokeStyle=stroke;ctx.fillStyle=fill;ctx.lineWidth=2;ctx.fill();ctx.stroke();if(label){ctx.fillStyle=stroke;ctx.font="bold 12px system-ui";ctx.fillText(label,ox+quad[0]*scale+3,oy+quad[1]*scale-4);}}
function canvasToImage(event){if(!state.view)return null;const rect=el.editorCanvas.getBoundingClientRect();const x=(event.clientX-rect.left)*(el.editorCanvas.width/rect.width),y=(event.clientY-rect.top)*(el.editorCanvas.height/rect.height);return{x:(x-state.view.offsetX)/state.view.scale,y:(y-state.view.offsetY)/state.view.scale};}
