...
const TEXT = {
  ...
  ocrLayout: ["OCR / Textlayout", "OCR / text layout"],
  ...
  localizeAlias: ["Alias innerhalb längerer OCR-Zeile lokalisieren", "Locate alias inside a longer OCR line"],
  ...
  validationMinAnchor: ["Mindest-Ankerquote für gültiges OCR-Label", "Minimum anchor score for a valid OCR label"],
  ...
  masterOcr: ["Masterbild und PaddleOCR", "Master image and PaddleOCR"],
  ocrPreparing: ["PaddleOCR wird vorbereitet …", "Preparing PaddleOCR …"],
  loadMaster: ["Masterbild laden", "Load master image"],
  runOcr: ["PaddleOCR auf Masterbild", "Run PaddleOCR on master image"],
  exportOcr: ["OCR-Ergebnis exportieren", "Export OCR result"],
  cancelAnalysis: ["Analyse abbrechen", "Cancel analysis"],
  hideOcr: ["OCR-Boxen ausblenden", "Hide OCR boxes"],
  removeMaster: ["Masterbild entfernen", "Remove master image"],
  firstStartModel: ["Erster Start lädt PP-OCRv5 mobile.", "First start loads PP-OCRv5 mobile."],
  selectOcrBox: ["OCR-Box auswählen", "Select OCR box"],
  ...
  masterHint: ["Masterbild laden. Dann PaddleOCR starten oder direkt eine freie Zone zeichnen.", "Load a master image. Then run PaddleOCR or draw a free zone directly."],
  noMaster: ["Noch kein Masterbild", "No master image yet"],
  currentSelection: ["Aktuelle Auswahl", "Current selection"],
  noSelection: ["Keine OCR-Box oder freie Zone ausgewählt.", "No OCR box or free zone selected."],
  ...
  fieldLabel: ["Bezeichnung", "Label"],
  finalRegex: ["Regex für den fertigen Wert", "Regex for final value"],
  sourceRegex: ["Regex für den OCR-Rohtext", "Regex for OCR raw text"],
...
const HELP = {
  ...
  recognitionSource: ["OCR wertet Text und dessen Position aus. QR liest einen QR-Code nach den hinterlegten Parserregeln.", "OCR evaluates text and its position. QR reads a QR code using the configured parser rules."],
  ...
  localizeAlias: ["Wenn PaddleOCR eine längere Zeile erkennt, wird nur der Teilbereich des Alias als Anker benutzt. Hilfreich bei 'Prüflos 12345'.", "If PaddleOCR recognizes a longer line, only the alias portion is used as the anchor. Useful for 'Inspection Lot 12345'."],
  ...
  validationMinAnchor: ["Mindest-Ankerquote, damit ein bereits gewähltes OCR-Profil als gültiges Label akzeptiert wird.", "Minimum anchor score for an already selected OCR profile to be accepted as a valid label."],
  ...
  fieldLabel: ["Anzeigename des Feldes.", "Display name of the field."],
  ...
  sourceRegex: ["Filtert OCR-Rohtexte, bevor Bereinigung und weitere Strategien angewendet werden.", "Filters OCR raw text before cleanup and other strategies are applied."],
  cleanup: ["Wandelt den gefundenen OCR-Text in den endgültigen Feldwert um. Die verfügbaren Optionen hängen vom Feldtyp ab.", "Converts recognized OCR text into the final field value. Available options depend on the field type."],
  lastDigitsCount: ["Anzahl der Ziffern, die bei 'Letzte Ziffern übernehmen' vom Ende des OCR-Textes genommen werden.", "Number of digits taken from the end when 'Keep trailing digits' is selected."],
  searchRadius: ["Wie weit um die erwartete Feldzone herum nach einem passenden OCR-Kandidaten gesucht werden darf. Standard ist 1,8.", "How far around the expected field zone matching OCR candidates may be searched. Default is 1.8."],
  minOverlap: ["Mindestanteil, mit dem eine OCR-Box die erwartete Feldzone überlappen muss. 0 erlaubt auch nahe Kandidaten ohne Überlappung.", "Minimum fraction by which an OCR box must overlap the expected field zone. 0 also allows nearby candidates without overlap."],
  ...
  strategyUnits: ["Einheiten, die eine spezielle Gewichtsstrategie beim Zusammensetzen oder Bewerten von OCR-Kandidaten bevorzugt, z. B. KGM, LTR.", "Units a special weight strategy should prefer when combining or scoring OCR candidates, e.g. KGM, LTR."],
